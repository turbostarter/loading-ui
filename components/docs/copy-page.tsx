...
function getPromptUrl(baseURL: string, url: string) {
  return `${baseURL}?q=${encodeURIComponent(
    `I’m looking at this loading-ui documentation: ${url}.
...
export function DocsCopyPage({ page, url }: { page: string; url: string }) {
  ...
  const trigger = (
    <Button
      variant="secondary"
      size="icon-sm"
      className="peer -ml-0.5 shadow-none md:text-[0.8rem]"
    >
    ...
    <Popover>
      <div className="group/buttons bg-secondary relative flex rounded-lg *:[[data-slot=button]]:focus-visible:relative *:[[data-slot=button]]:focus-visible:z-10">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            className="w-52 rounded-lg shadow-none"
          >
        ...
        <Separator
          ...
          className="bg-foreground/5! absolute top-1 right-8 z-0 h-6! peer-focus-visible:opacity-0 sm:right-7"
        />
